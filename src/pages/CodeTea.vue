<template>
  <div v-for="product in products" :key="product.name">
    ...
    <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
      ...
      <template v-if = "product.images.length > 3">
      ...
      <div v-else class="aspect-h-4 aspect-w-3 hidden overflow-hidden rounded-lg lg:block">
        <img :src="product.images[1].src" :alt="product.images[1].alt"
          class="h-full w-full object-cover object-center" />
      </div>

      <template v-if = "product.images.length > 4">
        <div class="hidden lg:grid lg:grid-cols-1 lg:gap-y-8">
          <div class="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg">
          ...
          <div class="aspect-h-2 aspect-w-3 overflow-hidden rounded-lg">
            <img :src="product.images[4].src" :alt="product.images[4].alt"
              class="h-full w-full object-cover object-center" />
          </div>
        </div>
      </template>
      <div v-else class="aspect-h-4 aspect-w-3 hidden overflow-hidden rounded-lg lg:block">
        <img :src="product.images[product.images.length-1].src" :alt="product.images[product.images.length-1].alt"
          class="h-full w-full object-cover object-center" />
      </div>
    </div>
    ...
    <div
      ...
      <div class="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">
          <a v-if="product.link" href="https://en.wikipedia.org/wiki/Da_Hong_Pao" class="underline underline-offset-8" target="_blank">{{ product.name }}</a>
        ...
        <form class="mt-10">
          ...
          <div>
            <div class="flex items-center justify-between">
              <h3 class="text-sm font-medium text-gray-900">Standard</h3>
            </div>

            <fieldset aria-label="Choose a standard" class="mt-4">
              <RadioGroup v-model="product.standard"
                class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                <RadioGroupOption as="template" v-for="size in product.standards" :key="size.name" :value="size.name"
          ...
          <template v-if="product.inBag">
            <button type="button" @click="$router.push('/ShoppingBag')"
              class="mt-10 flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
              Settlement
            </button>
            <button type="button" @click="product.inBag=false"
          ...
          <button v-else type="button" @click="product.inBag=true"
            class="mt-10 flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
            Add to bag
          </button>
        </form>
      ...
      <div class="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
        ...
        <div class="mt-10">
          ...
          <ul role="list" class="list-disc space-y-2 pl-4 text-sm">
            <li v-for="item in product.features" :key="item.label" class="text-gray-400">
...
import type { IProduct } from '@/store/state';
...
watch(route, (to) => {
  category_name.value = to.name as string
}, { immediate: true })


const products = computed(() => {
  return store.products.filter(item => item.category === category_name.value)
})

const selectedPrice = (product: IProduct) => {
  const price = product.standards.find(item => item.name === product.standard)?.price || 0
  return price * product.quantity! / product.defaultQuantity
}
